...
$gpu.Text = "GPU (nnedi3cl)"; $gpu.Location = New-Object System.Drawing.Point(100, $y)
$gpu.Size = New-Object System.Drawing.Point(120, 20); $gpu.Checked = $true
$cpu = New-Object System.Windows.Forms.RadioButton
$cpu.Text = "CPU (znedi3)"; $cpu.Location = New-Object System.Drawing.Point(220, $y)
$cpu.Size = New-Object System.Drawing.Point(100, 20)
$form.Controls.Add($gpu); $form.Controls.Add($cpu)
$y += 28
...
$templatePath = Join-Path $scriptDir "VapourSynth Templates\restore.vpy"
$outDir = Join-Path $scriptDir "Source"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null

foreach ($f in $files) {
    $baseName = [System.IO.Path]::GetFileNameWithoutExtension($f)
    $outFile = Join-Path $outDir "$baseName.vpy"
    $c = (Get-Content $templatePath -Raw) `
        -replace '\[CLIP\]', $f `
        -replace '\[CLIP-NO-EXTENSION\]', [System.IO.Path]::ChangeExtension($f, $null) `
        -replace '\[FIELD\]', $fieldVal `
        -replace '\[TFF\]', $tffVal `
        -replace '\[CROP\]', $cropVal `
        -replace '\[DENOISE\]', $denoiseVal `
        -replace '\[UPSIZER\]', $upsizer `
        -replace '\[WIDTH\]', $w `
        -replace '\[HEIGHT\]', $h
    [System.IO.File]::WriteAllText($outFile, $c, [System.Text.Encoding]::UTF8)
    Write-Host "Created: Source\$baseName.vpy ($w`x$h, $darVal)"
}
